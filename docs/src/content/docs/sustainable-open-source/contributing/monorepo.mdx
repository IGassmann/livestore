...
import { EFFECT_VERSION } from '../../../../../../packages/@local/shared/src/CONSTANTS'
...
- [GitHub CLI](https://cli.github.com/) for release and issue tasks
...
Test defaults are loaded from the closest owning package's `.env.test` file
(for example `tests/integration/.env.test` for Playwright integration tests).
For local overrides (credentials, custom service endpoints), export them in
your shell or put them in an untracked `.env.test.local` file in that package
before running the relevant `pnpm run ...` command.
...
- This project uses [pnpm](https://pnpm.io/) to manage the workspace.
- We're using the `workspace:` protocol (normally `workspace:^`) to link packages together.
...
- For updating dependencies across the monorepo, see
  ...
  (manual version alignment, peer-dependency handling, and validation).
...
- `react`/`react-dom`, whose compatibility must be checked against the Expo and
  React Native versions used by the documentation snippets
