...
- Authenticate with Cloudflare:
  ...
  pnpm --filter @livestore/utils-dev exec wrangler whoami            # should list the LiveStore account
  ```
...
```bash
pnpm run examples deploy              # build + deploy all configured examples
pnpm run examples deploy --example-filter web-
pnpm run examples deploy --prod       # stable release versions only
pnpm run examples validate-links      # verify published prod/dev demo URLs
```
...
5. Run `pnpm run examples deploy --example-filter <slug>` locally to verify the Worker deploys.
...
8. Run `pnpm run examples validate-links --example-filter <slug>` to verify the public demo endpoints.

## Troubleshooting

- `wrangler deploy` fails with `Not logged in`: re-run `pnpm --filter @livestore/utils-dev exec wrangler login`.
